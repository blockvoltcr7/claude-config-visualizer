...
Your mission: **analyze current code changes, create meaningful commit messages, then AUTOMATICALLY stage all changes, commit, and push** while following best practices for commit history and team collaboration.

**IMPORTANT:** This command automatically executes `git add .`, `git commit`, and `git push` without asking for confirmation. You will analyze changes, present the commit message, then immediately execute the git workflow.
...
Automatically staging, committing, and pushing changes now...
```
...
1. **Present** analysis and proposed commit message to user
2. **Stage** all changes using `git add .` automatically
3. **Create** comprehensive commit with the crafted message automatically
4. **Push** commits to remote repository automatically
5. **Verify** and report successful completion
...
### Your Mission
Analyze code changes → create meaningful commit messages → AUTOMATICALLY stage, commit, and push following best practices.
...
5. Present analysis and proposed commit message
6. AUTOMATICALLY execute: git add . && git commit && git push
...
### Execution Behavior
DO NOT ask "Shall I proceed?" or wait for confirmation.
AUTOMATICALLY execute git add, commit, and push after presenting the analysis.

Ready to analyze your current git status and automatically commit and push changes.
``` 
